...
function builtTruffleContractLoader(name) {
    try {
        const directoryPath = "@superfluid-finance/ethereum-contracts/build/contracts";
...
module.exports = async function (callback, argv, options = {}) {
    try {
        ...
        const tokenFactoryAddress = await sf.host.getSuperTokenFactory();
        console.log('tokenFactoryAddress', tokenFactoryAddress);
        const superTokenFactory = await sf.contracts.ISuperTokenFactory.at(tokenFactoryAddress);
        ...
        if (tokenName == sf.config.nativeTokenSymbol) {
            ...
            deploymentFn = async () => {
                ...
                const superTokenName = `Super ${tokenName}`;
                const superTokenSymbol = `${tokenSymbol}x`;
                console.log('New SuperToken =>', superTokenName, '-', superTokenSymbol);

                const wrapperAddress = await superTokenFactory.callStatic['createERC20Wrapper(address,uint8,string,string)'](
                    tokenInfo.address,
                    "1", // upgradability
                    superTokenName,
                    superTokenSymbol,
                ...
                const tx = await superTokenFactory['createERC20Wrapper(address,uint8,string,string)'](
                    tokenInfo.address,
                    "1", // upgradability
                    superTokenName,
                    superTokenSymbol
                ...
                const superToken = await sf.contracts.ISuperToken.at(wrapperAddress);
                superToken.tx = tx;
                return superToken;
            };
        }
        ...
        console.log("SuperToken name at the resolver: ", name);
        ...
        if (doDeploy) {
            ...
            console.log(`Resolver setting new address for ${name}...`);
            const testResolver = await TestResolver.at(sf.resolver.address);
            // await testResolver.contract.methods.set(name, superToken.address);
            await testResolver.set(name, superToken.address);
            console.log('testResolver', name, await testResolver.get(name));
